    ...
    <div className="space-y-6">
      {!parsedTransactions.length ? (
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6">
          <h3 className="text-lg font-medium text-slate-200 mb-4">Upload Bank Statement</h3>
          
          <div className="border-2 border-dashed border-slate-600 rounded-xl p-8 text-center bg-slate-800/50 hover:bg-slate-700/50 transition-colors">
            ...
            {file && <p className="mt-4 text-sm text-blue-400 font-medium">Selected: {file.name}</p>}
          </div>

          {error && (
            <div className="mt-4 p-3 bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-lg text-sm flex items-center gap-2">
                ...
                {parsedTransactions.map((tx, idx) => (
                  <tr key={idx} className="hover:bg-slate-700/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`px-2 py-1 rounded text-xs ${tx.type === 'income' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                        {tx.type}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <select
                        ...
                        className="bg-slate-900 border border-slate-700 text-slate-300 rounded-lg px-2 py-1 focus:ring-blue-500 focus:border-blue-500 w-full text-xs"
                      >
